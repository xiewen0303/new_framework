 ...
 */
@Service
public class GuildBeiDongSkillConfigExportService extends AbsClasspathConfigureParser {

	
	/**
	 * 配置名
	 */
	private String configureName = "MenPaiBeiDongJiNengBiao.jat";
	private Map<String,GuildBeiDongSkillConfig> configs;
	
	@SuppressWarnings("unchecked")
	protected void configureDataResolve(byte[] data) {
		Map<String,GuildBeiDongSkillConfig> configs = new HashMap<>();
		...
		for (Object obj:skillListData) {
			...
			if (null != tmp) {
				GuildBeiDongSkillConfig skillConfig = createGuildBeiDongSkillConfig(tmp);
				if(skillConfig == null){
					continue;
				}
	 ...
	 * @return
	 */
	private GuildBeiDongSkillConfig createGuildBeiDongSkillConfig(Map<Object, Object> tmp) {
		String skillId = CovertObjectUtil.object2String(tmp.get("id"));
		if(CovertObjectUtil.isEmpty(skillId)){
			return null;
		}
		GuildBeiDongSkillConfig config = new GuildBeiDongSkillConfig();
	...
	public GuildBeiDongSkillConfig loadById(String skillId) {
