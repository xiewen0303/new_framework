 ...
 * 2015年04月28日
 */
public class BaguaFbStage  extends AoiStage{
	 ...
	 */
	public BaguaFbStage(String id, Integer mapId,AOIManager aoiManager, PathInfoConfig pathInfoConfig,StageType stageType,
			ReadOnlyMap<String, Integer> needKillMap,int expireDelay,short exitCmd,int fubenId,int ceng) {
	...
	public void startScheduleExpireCheck(Long userRoleId){
		...
		getScheduleExecutor().schedule(userRoleId.toString(), GameConstants.COMPONENT_BAGUA_FUBEN_FORCED_LEAVE, runable, getExpireDelay(), TimeUnit.MILLISECONDS);
	}
	...
	public void cancleScheduleExpire(Long userRoleId){
		getScheduleExecutor().cancelSchedule(userRoleId.toString(), GameConstants.COMPONENT_BAGUA_FUBEN_FORCED_LEAVE);
	}
...
//		clearFubenMonster();
		if(challengers!=null){
			for(IRole role : challengers){
				...
				getScheduleExecutor().schedule(role.getId().toString(), GameConstants.COMPONENT_BAGUA_FUBEN_FORCED_LEAVE, runable, getExpireCheckInterval(), TimeUnit.MILLISECONDS);
			}
		}
	...
	public int getExpireCheckInterval(){
		return GameConstants.BAGUA_FUBEN_EXPIRE_CHECK_INTERVAL;
	}
...
//	}
	...
	public void noticeClientExit(Long userRoleId){
		KuafuMsgSender.send2KuafuSource(userRoleId, ClientCmdType.BAGUA_LEAVE_FUBEN, AppErrorCode.OK);
	}
