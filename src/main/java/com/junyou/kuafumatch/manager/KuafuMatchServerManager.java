...
public class KuafuMatchServerManager {
	...
	private static void initPool() {
		...
		String kuafuMatchServerId = redis.get(RedisKey.KUAFU_MATCH_SERVER);
		if (kuafuMatchServerId == null) {
			GameLog.error("kuafuMatchServerId is null ");
			return;
		}
		...
		GameLog
				.info("kuafu match server connection pool init ip={},port={},serverId={}",
	...
	public static void register() {
		...
		String matchServerId = redis.get(RedisKey.KUAFU_MATCH_SERVER);
		if(matchServerId!=null && !matchServerId.equals(serverId)){
			throw new RuntimeException("多次注册匹配服务器错误，请确保匹配服务器只有一台！如果想变更匹配服务器，请先修改redis内的"+RedisKey.KUAFU_MATCH_SERVER+"的值");
		}
		
		if (redis.exists(RedisKey.buildKuafuServerKey(serverId))) {
			redis.set(RedisKey.KUAFU_MATCH_SERVER, serverId);
